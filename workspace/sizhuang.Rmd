...
# knitr::kable(clean_data[1:5, 1:6], caption = "Cleaned Data")
```

```{r}
columns_to_convert <- c("growth rate CPUE", "temp.PREINC", "temp.INC", "temp.HATCH", "temp.BROOD", "temp.WINTER", "temp.SPRING", "prec.PREINC", "prec.INC", "prec.HATCH", "prec.BROOD")
# Convert these columns to numeric
clean_data[columns_to_convert] <- lapply(clean_data[columns_to_convert], as.numeric)

# Check for any remaining NAs
# sapply(clean_data[columns_to_convert], function(x) sum(is.na(x)))
```
...
p1 <- ggplot(clean_data, aes(x=year, y=CPUE)) +
  geom_line(color="steelblue") + 
  geom_point() + 
  geom_rect(
    data = subset(clean_data, peak_rodent_year == 'yes'),
...
p2 <- ggplot(clean_data, aes(x=year, y=log.CPUE)) + 
  geom_line(color="steelblue") + 
  geom_point() + 
  geom_rect(
    data = subset(clean_data, peak_rodent_year == 'yes'),
...
```{r ACF, echo=FALSE}
acf(clean_data$log.CPUE, lag = 100, main = "ACF log.CPUE")
```

```{r PACF, echo=FALSE}
pacf(clean_data$log.CPUE, lag = 100, main = "PACF log.CPUE")
```
...
```{r}
# Box plot of CPUE for each Peak Rodent Year
ggplot(data, aes(x = peak_rodent_year, y = CPUE, fill = peak_rodent_year)) +
  geom_boxplot() +
  labs(title = "CPUE by Peak Rodent Year", x = "Peak Rodent Year", y = "CPUE")
```
