...
![Model parameters over iterations](./assets/local_search_1.png)
...
```{r, echo=FALSE}
results <- read.csv('/Users/ruojunliu/Desktop/bird_params_small.csv')
...
The best estimated log likelihood under global search by the use of GreatLakes is -320.4, which is a lot lower than the log likelihood from ARIMA (log-likelihood with -99) and local search(log-likelihood with -134). It is opposite to our expectation. One of the reasons is that we limit the parameter space to save the running time on Great Lakes, otherwise it will take more than 8 hours to run given the fact that our model is built with a large number of parameters. The accuracy will be increased if we increase the parameter space. 
...
- Heyuan: Wrote the interpretation of result, do the POMP, local search and global search. 
- Sizhuang: Helped with POMP, local search and global search and created visualizations of some of the POMP results
Professor and GSI helped us a lot in building POMP model.
--------
